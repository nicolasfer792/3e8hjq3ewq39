...
export async function createReservation(
  ...
  const supabase = getSupabaseServer()
  // Cargar config
  const { data: cfgRows, error: cfgLoadErr } = await supabase.from("pricing_config").select("*").limit(1)
  ...
  const reservationRow = {
    ...
    deleted_at: null, // Asegurarse de que no esté en papelera al crear
  }
  ...
  // Registrar el gasto de limpieza
